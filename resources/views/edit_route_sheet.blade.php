...
<div class="container">
    ...
    @endif
    <form action="/operator/edit/route/sheet/{{ $data->id }}" method="post" id="form_id">
        @csrf
        <div class="d-div">
            ...
            <select name="doctor_id" id="doctor_select">
                @if(!empty($doctors))
                    @foreach($doctors as $doctor)
                        <option value="{{ $doctor['id'] }}"{{ $doctor['id'] == $data->doctor_id ? ' selected' : '' }}>{{ $doctor['fio'] }}</option>
                    @endforeach
                @endif
            </select>
        </div>
        <div class="d-div">
            ...
            <select name="srochno" id="srochno">
                <option value="0"{{ $data->srochno == 0 ? ' selected' : '' }}>Не срочно</option>
                <option value="1"{{ $data->srochno == 1 ? ' selected' : '' }}>Срочно</option>
                <option value="2"{{ $data->srochno == 2 ? ' selected' : '' }}>Очаг</option>
            </select>
        </div>
        <div class="d-div">
        ...
        <div class="d-div">
            ...
            <select name="route_sheet_status" id="application_status">
                <option value="Новая"{{ $data->route_sheet_status == 'Новая' ? ' selected' : '' }}>Новая</option>
                <option value="Выполнена"{{ $data->route_sheet_status == 'Выполнена' ? ' selected' : '' }}>Выполнена</option>
                <option value="Отменена"{{ $data->route_sheet_status == 'Отменена' ? ' selected' : '' }}>Отменена</option>
            </select>
        </div>
        <div class="big-split"></div>
